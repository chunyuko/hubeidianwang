...
import { columns, parseFields } from "@/config/options/develop.js";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      loadData: (parameter) => {
        ...
        return this.$store.dispatch("list_develop", requestParameters).then((res) => {
  ...
  methods: {
    ...
    limitMulVideo(vals, form, paths) {
      ...
      let width = res.vwidth;
      let height = res.vheight;
    ...
    limitVideo(vals, form, path) {
      let res = JSON.parse(localStorage.getItem("admin"));
      let width = res.width;
      let height = res.height;
      let videos = getVideo(vals.get(path)).then((res) => {
        console.log(res.name, res.type, res.width, res.height, res.size, res.duration);
        return res;
      });
      return new Promise((resolve, reject) => resolve(videos)).then((res) => {
        let failList = [];
        if (res.width > width || res.height > height) {
          failList.push(res);
          // throw new Error(`${res.name}像素需要在${width}*${height}内,请删除后在上传该视频`);
          this.$message.error(res.name + `像素需要在${width}*${height}内,请删除后再上传该视频`);
        }
        if (failList.length == 0) {
          console.log("开始上传FormData数据", vals);
    ...
    oked(vals, form) {
      if (this.view_next && !this.view_more) {
        vals.set("developData", this.selRecord.id);
        console.log(">");
    ...
    sumbit(vals, type, form) {
      ...
      if (this.view_next && !this.view_more) {
        ...
        // vals["refresh"] = true;
        // console.log(">>>>>>>>>", vals);
        return this.$store.dispatch(type + "_develop", vals).then((res) => {
          // this.onClose();
    ...
    delMedia() {
      if (this.checkednextKeys.length > 0 && this.view_next && !this.view_more) {
        this.asking = true;
        this.sumbit({ ids: this.checkednextKeys, developData: this.selRecord.id }, "del_multi");
        console.log("..");
