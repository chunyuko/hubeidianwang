  ...
  data() {
    ...
    return {
      ...
      loadData: (parameter) => {
        ...
        return this.$store.dispatch("list_manage", requestParameters).then((res) => {
  ...
  methods: {
    ...
    sumbit(vals, type, form) {
      ...
      if (this.view_next && !this.view_more) {
        ...
        // vals["refresh"] = true;
        // console.log(">>>>>>>>>", vals);
        return this.$store.dispatch(type + "_manage", vals).then((res) => {
          // this.onClose();
    ...
    delMedia() {
      if (this.checkednextKeys.length > 0 && this.view_next && !this.view_more) {
        this.asking = true;
        this.sumbit({ ids: this.checkednextKeys, manageData: this.selRecord.id }, "del_multi");
        console.log("..");
